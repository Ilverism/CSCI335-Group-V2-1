...
NUM_IMAGES = 30     #<- Images per country code     (total = NUM_IMAGES * |FLAG_CODES|)


#Possible background types
BACKGROUND_TYPES = ["solidorgradient", "realistic", "noise"]
...
def download_image(url):
    ...
    while True:
        ...
        try:
            ...
            print(f"Error downloading image: {response.status_code} - {response.reason}... Retrying in 10 seconds")
            time.sleep(10)
            continue
...
def main():
    ...
    for i in range(NUM_IMAGES):
        for code in FLAG_CODES:
            ...
            filename = f"flag_{code}_{bg_type}.jpg"
            
            #Append index if file already exists with the same name
            index = 1
            while os.path.exists(os.path.join(OUTPUT_DIR, filename)):
                filename = f"flag_{code}_{bg_type}_{index}.jpg"
                index += 1
